...
def plot_velfield(x, y, vel, vmin=None, vmax=None, ncolors=64, nodots=False, colorbar=False, label=None,
                  flux=None, fixpdf=False, nticks=7, **kwargs):
    ...
    cs = ax.tricontourf(x, y, vel.clip(vmin, vmax), levels=levels, cmap=kwargs.get("cmap", sauron))
    ...
    if fixpdf:  # remove white contour lines in PDF at expense of larger file size
        ax.tricontour(x, y, vel.clip(vmin, vmax), levels=levels, zorder=0, cmap=kwargs.get("cmap", sauron))
